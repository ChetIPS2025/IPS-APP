...
def _render_saved_timesheets_table(job_id: str, *, key_prefix: str, week_start: date) -> None:
    ...
    for i, row in enumerate(rows[:4]):
        ...
        with act_cols[i % len(act_cols)]:
            ...
            if can_manage_weekly_timesheets(current_role()) and not timesheet_is_locked(row):
                if st.button("Mark Sent", key=f"{key_prefix}_sent_{tid}", use_container_width=True):
                    mark_timesheet_sent(tid)
                    st.rerun()
            if can_manage_weekly_timesheets(current_role()) and status not in {"Approved", "Signed", "Voided"}:
...
def render_weekly_timesheet_builder(
    *,
    ...
    if not job_id:
        ...
        for lab, jid in jobs.items():
            if str(jid) == str(default_job_id):
                default_label = lab
                break
        pick = st.selectbox("Job", labels, index=labels.index(default_label) if default_label in labels else 0, key=f"{kp}_job")
    ...
    if preview or st.session_state.get(f"{kp}_show_preview"):
        ...
        components.html(html_doc, height=640, scrolling=True)
        st.download_button(
