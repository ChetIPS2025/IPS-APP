...
def _render_add_materials_form(job: dict, *, key_prefix: str) -> None:
    ...
    tab_scan, tab_inventory, tab_manual = st.tabs(["Scan / lookup", "From inventory", "Manual entry"])

    common_used_date = st.date_input("Date used", value=date.today(), key=f"{key_prefix}_used_date")
    common_used_time = st.time_input("Time used", value=datetime.now().time().replace(second=0, microsecond=0), key=f"{key_prefix}_used_time")
    ...
    with tab_manual:
        st.caption("Use for materials not in inventory (field purchase, misc. consumables).")
