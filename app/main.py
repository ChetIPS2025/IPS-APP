...
try:
    from app.styles import (
        ...
        inject_global_css as inject_ips_foundation_css,
...
def _inject_app_shell(*, login_only: bool = False) -> None:
...
def _on_login_success() -> None:
    """Persist session, route to Dashboard, rerun so login widgets are not on screen."""
    persist_auth_cookies_if_pending()
    set_nav_slug("dashboard")
    st.session_state[SESSION_NAV_KEY] = "dashboard"
    log_auth_state("login_rerun_dashboard")
    st.rerun()


def _render_login_screen() -> None:
    """Login UI only — must be followed by st.stop() or st.rerun()."""
    ...
    if _pend:
        st.info(
            "You opened an **inventory scan** link. Sign in below, then we will take you to "
            "**Scan Inventory** for that code."
        )
...
def _render_password_reset_gate() -> None:
...
def _render_authenticated_app() -> None:
    """Sidebar + module routing — only when logged in."""
    with perf_span("main.page_routing"):
        apply_pending_navigation()
        sync_session_route_slug_to_nav_page()
        legacy_page = st.session_state.get(IPS_NAV_PAGE_KEY)
        if legacy_page:
            set_nav_slug(normalize_nav_slug(str(legacy_page)))
...
def main() -> None:
    ...
    if token and str(token).strip():
        _inject_app_shell(login_only=False)
        try:
            try:
    ...
    # --- Auth gate: login OR app shell, never both on one run ---
    if not is_authenticated():
        _inject_app_shell(login_only=True)
