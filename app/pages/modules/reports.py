...
def _report_block(title: str, rows: list[dict[str, Any]], columns: list[tuple[str, str]], *, csv_name: str, key: str, cell_fn=None) -> None:
    ot = "d" + "iv"
    ...
    render_data_table(
        rows,
        columns,
        row_id_key="id" if rows and "id" in rows[0] else list(rows[0].keys())[0] if rows else "id",
        selected_id=None,
...
def render() -> None:
    inject_global_css()
    render_page_header("Reports", "Operational summaries with CSV export placeholders.")
    ...
    open_est = [
        {
            "id": str(r.get("estimate_number")),
            **r,
            "total": fmt_currency(r.get("total")),
        }
