...
# Query param used with st.image(link=...) so clicking a thumbnail opens the large preview below.
    ...
    *,
    ...
    if kind == "image":
        try:
            ...
            if is_http:
                st.link_button("Open file", url=ref, key=f"{key_base}_img_fallback")
            else:
                p = Path(ref)
                if p.is_file():
                    st.download_button(
    ...
    if kind == "pdf":
        st.markdown("##### 📄")
        if is_http:
            st.link_button(file_name[:40] + ("…" if len(file_name) > 40 else ""), url=ref, key=f"{key_base}_pdf")
        else:
            p = Path(ref)
            if p.is_file():
                st.download_button(
    ...
    st.markdown("##### 📎")
    if is_http:
        st.link_button(file_name[:40] + ("…" if len(file_name) > 40 else ""), url=ref, key=f"{key_base}_oth")
    else:
        p = Path(ref)
        if p.is_file():
            st.download_button(
