...
def _render_jobs_table(rows: list[dict], *, selected_id: str) -> None:
    ...
    with st.container(border=True):
        st.markdown(
            '<span class="ips-clean-table ips-jobs-click-table ips-data-table-anchor" aria-hidden="true"></span>',
            unsafe_allow_html=True,
        )
        st.markdown(
        ...
        for row_idx, job in enumerate(rows):
            ...
            label = str(job.get("job_number") or job.get("job_name") or "job")
