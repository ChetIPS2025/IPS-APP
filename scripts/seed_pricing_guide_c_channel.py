#!/usr/bin/env python3
"""Idempotent seed: C-channel materials (Carbon Steel, Aluminum)."""
...
def main() -> int:
    ...
    for row in items:
        ...
        ok, msg = save_pricing_item(payload, row_id=row_id, changed_by="seed_c_channel")
        if not ok:
    ...
    print(
        f"C-channel seed complete: {created} created, {updated} updated, "
        f"{c_channel_source_row_count()} source rows -> {len(items)} catalog items."
    )
    if review:
