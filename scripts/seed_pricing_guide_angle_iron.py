#!/usr/bin/env python3
"""Idempotent seed: angle iron materials (Carbon Steel, Aluminum, Stainless Steel)."""
...
def main() -> int:
    ...
    for row in items:
        ...
        ok, msg = save_pricing_item(payload, row_id=row_id, changed_by="seed_angle_iron")
        if not ok:
    ...
    clear_pricing_guide_cache()
    missing = angle_iron_requires_price_review()
    active = angle_iron_active_catalog_items()
    print(
        f"Angle iron seed complete: {created} created, {updated} updated, "
        f"{flagged} flagged inactive (missing price), "
        f"{angle_iron_source_row_count()} source rows -> {len(items)} catalog items "
