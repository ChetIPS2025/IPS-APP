#!/usr/bin/env python3
"""Idempotent seed: flat bar materials (Stainless Steel, Aluminum, Carbon Steel)."""
...
def main() -> int:
    ...
    for row in items:
        ...
        ok, msg = save_pricing_item(payload, row_id=row_id, changed_by="seed_flat_bar")
        if not ok:
    ...
    print(
        f"Flat bar seed complete: {created} created, {updated} updated, "
        f"{flat_bar_source_row_count()} source rows -> {len(items)} catalog items."
    )
    if review:
