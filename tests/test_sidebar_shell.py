...
def test_sidebar_width_tokens_match_design_spec():
    ...
    assert IPS_SIDEBAR_COLLAPSED_LOGO_PX == 40
    assert IPS_SIDEBAR_EXPANDED_LOGO_PX == 52
...
def test_desktop_nav_rail_css_hides_legacy_streamlit_wrap():
    ...
    assert "ips-desktop-nav-rail-v6" in css
    assert f"left: {IPS_NAV_RAIL_GUTTER_PX}px" in css
    assert (
        f"margin-left: {IPS_SIDEBAR_COLLAPSED_WIDTH_PX + IPS_NAV_RAIL_GUTTER_PX + IPS_NAV_RAIL_CONTENT_GAP_PX}px"
        in css
    )
